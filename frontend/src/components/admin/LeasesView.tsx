...
export function LeasesView({ leases, units, onCreate, onChange }: { leases: AdminLease[]; units: AdminUnit[]; onCreate: (value: AdminLease) => void; onChange: (value: AdminLease) => void }) {
  ...
  function startCreate() { setEditingId(null); setForm({ ...EMPTY, property_id: units[0]?.id ?? "" }); setMessage(""); }
  function startEdit(lease: AdminLease) { setEditingId(lease.id); setForm(formFromLease(lease)); setMessage(""); }
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editingId) { const { property_id: _propertyId, ...update } = form; void _propertyId; onChange(await updateAdminLease(editingId, update)); setMessage("Đã cập nhật hợp đồng."); }
      else { onCreate(await createAdminLease(form)); setMessage("Đã tạo hợp đồng."); setForm({ ...EMPTY, property_id: units[0]?.id ?? "" }); }
