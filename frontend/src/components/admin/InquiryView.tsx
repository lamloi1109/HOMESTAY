...
export function InquiryView({ inquiries, onChange }: { inquiries: AdminInquiry[]; onChange: (value: AdminInquiry) => void }) {
  ...
  const rows = useMemo(() => inquiries.filter((item) => {
    ...
    return stageMatch && (!needle || [item.guest_name, item.phone, item.email, item.unit_code, item.property_name].some((value) => value?.toLocaleLowerCase("vi").includes(needle)));
  ...
  function changeSearch(value: string) { setSearch(value); setSelectedId(""); setDraftStage(null); setDraftNote(null); setMessage(""); }
  async function save() {
    if (!selected) return;
  ...
  return <div className="grid gap-6">
    <div className="flex flex-wrap gap-3 items-center justify-between">
      <div className="flex flex-wrap gap-2" aria-label="Lọc theo trạng thái">{STAGES.map((item) => { const count = item.value === "all" ? inquiries.length : inquiries.filter((inq) => inq.stage === item.value).length; return <button key={item.value} type="button" aria-pressed={filter === item.value} onClick={() => changeFilter(item.value)} className={`min-h-11 px-4 border font-sans text-xs font-semibold uppercase tracking-[.12em] focus:outline-none focus:ring-2 focus:ring-[var(--gold-500)] ${filter === item.value ? "bg-[var(--jade-700)] border-[var(--jade-700)] text-[var(--gold-050)]" : "bg-transparent border-[var(--hairline-strong)] text-[var(--text-primary)]"}`}>{item.label} · {count}</button>; })}</div>
      <label className="relative min-w-[min(100%,260px)]"><span className="sr-only">Tìm yêu cầu</span><Icon name="search" size={16} className="absolute left-3 top-3.5 text-[var(--text-muted)]" /><input value={search} onChange={(event) => changeSearch(event.target.value)} placeholder="Tên, số điện thoại, mã căn…" className="w-full min-h-11 pl-10 pr-3 border border-[var(--hairline-strong)] bg-white font-sans text-sm focus:outline-none focus:ring-2 focus:ring-[var(--gold-500)]" /></label>
    </div>
    <div className="grid xl:grid-cols-[minmax(0,1fr)_340px] gap-6 items-start">
      <section className="border border-[var(--hairline-strong)] bg-[var(--surface-raised)]" aria-label="Danh sách yêu cầu">{rows.length === 0 ? <p className="p-8 font-italic italic text-[var(--text-muted)]">Không có yêu cầu phù hợp bộ lọc.</p> : <div className="grid gap-px bg-[var(--hairline)]">{rows.map((item) => <button key={item.id} type="button" aria-pressed={selected?.id === item.id} onClick={() => selectInquiry(item.id)} className={`p-4 sm:p-5 flex flex-wrap items-center gap-4 text-left focus:outline-none focus:ring-2 focus:ring-inset focus:ring-[var(--gold-500)] ${selected?.id === item.id ? "bg-[var(--canvas-warm)]" : "bg-[var(--surface-raised)] hover:bg-[var(--canvas)]"}`}><span className="px-2 py-1 bg-[var(--jade-700)] text-white font-sans text-[.625rem] font-semibold uppercase tracking-[.12em]">{CHANNEL_LABEL[item.channel]}</span><span className="flex-1 basis-44 min-w-0"><strong className="block font-display text-lg font-normal">{item.guest_name}</strong><span className="font-sans text-sm text-[var(--text-muted)]">{item.unit_code ?? item.property_name ?? "Chưa chọn căn"} · {item.rental_term ?? "Chưa rõ thời hạn"}</span></span><span className="font-sans text-xs text-[var(--text-body)]">{item.checkin_date ?? "Chưa chọn ngày"}</span><span className="font-sans text-[.6875rem] font-semibold uppercase tracking-[.12em] text-[var(--gold-900)]">{STAGE_LABEL[item.stage]}</span></button>)}</div>}</section>
      <aside className="border border-[var(--hairline-strong)] bg-[var(--canvas-warm)] p-5 sm:p-6 xl:sticky xl:top-6">{selected ? <div className="grid gap-5">
        ...
        <dl className="grid gap-3 border-y border-[var(--hairline)] py-4 font-sans text-sm"><div className="flex justify-between gap-4"><dt className="text-[var(--text-muted)]">Căn quan tâm</dt><dd className="text-right">{selected.unit_code ?? selected.property_name ?? "—"}</dd></div><div className="flex justify-between gap-4"><dt className="text-[var(--text-muted)]">Ngày nhận</dt><dd>{selected.checkin_date ?? "—"}</dd></div><div className="flex justify-between gap-4"><dt className="text-[var(--text-muted)]">Số khách</dt><dd>{selected.guest_count}</dd></div></dl>
