...
export interface ButtonProps extends React.ButtonHTMLAttributes<HTMLButtonElement> {
  variant?: "primary" | "gold" | "jade" | "outline" | "ghost" | "danger";
  size?: "sm" | "md" | "lg";
  icon?: string;
  iconAfter?: string;
  full?: boolean;
...
export function Button({
  ...
  // Styles based on Gao Ji House Design System v2: 0px radius, uppercase tracking, gold/jade accents
  const variantStyles = {
  ...
  if (as === "a" && href) {
    return (
      <a href={href} target={target} rel={rel} className={baseClasses} style={style}>
        {content}
      </a>
    );
  }
  ...
  return (
    <button type="button" disabled={disabled} className={baseClasses} style={style} {...props}>
      {content}
    </button>
  );
}
