...
export function AdminDashboard() {
  ...
  function openNewRequest() { setView("inbox"); }
  ...
  return <div className="admin-root min-h-screen bg-[var(--canvas)] text-[var(--ink-900)] flex flex-col lg:flex-row">
    <aside className="w-full lg:w-[260px] lg:min-h-screen lg:sticky lg:top-0 lg:self-start bg-[var(--jade-900)] text-[var(--gold-050)] flex flex-col border-r border-[rgba(212,175,55,.22)] shrink-0">
      ...
      <nav aria-label="Khu vực quản trị" className="p-0 flex lg:grid overflow-x-auto gap-px bg-[rgba(212,175,55,.16)]">
    ...
    <main className="flex-1 min-w-0">
      <header className="border-b border-[var(--hairline-strong)] bg-[var(--canvas-warm)] px-5 sm:px-8 lg:px-10 py-6 flex flex-wrap gap-4 items-end justify-between"><div><span className="font-sans text-[.6875rem] font-semibold uppercase tracking-[.18em] text-[var(--gold-900)]">{active.eyebrow}</span><h1 className="font-display text-3xl sm:text-4xl font-normal leading-tight mt-1">{active.title}</h1></div><div className="flex flex-wrap items-center gap-3"><span className="font-sans text-xs font-semibold uppercase tracking-[.14em] text-[var(--text-muted)]">{todayLabel}</span><Button type="button" variant="primary" size="sm" icon="calendar-check" onClick={openNewRequest}>Thêm Yêu Cầu Mới</Button></div></header>
